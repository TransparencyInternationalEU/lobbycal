...
angular
		.module(
				'lobbycalApp',
				[ 'LocalStorageModule', 'tmh.dynamicLocale',
						'pascalprecht.translate', 'ngResource', 'ngCookies',
						'ngAria', 'ngCacheBuster', 'ngFileUpload',
						'ui.bootstrap', 'ui.router', 'infinite-scroll',
						'angular-loading-bar' ])

		.run(
				function($rootScope, $location, $window, $http, $state,
						$translate, Language, Auth, Principal, ENV, VERSION) {
					// update the window title using params in the following
					// precendence
					// 1. titleKey parameter
					// 2. $state.$current.data.pageTitle (current state page
					// title)
					// 3. 'global.title'
					var updateTitle = function(titleKey) {
						if (!titleKey && $state.$current.data
					...
					$rootScope.$on('$stateChangeSuccess', function(event,
						...
						// Remember previous state unless we've been redirected
						// to login or we've just
						// reset the state memory after logout. If we're
						// redirected to login, our
						// previousState is already set in the
						// authExpiredInterceptor. If we're going
						// to login directly, we don't want to be sent to some
					...
					// if the current translation changes, update the window
					// title
