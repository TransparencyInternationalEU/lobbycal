...
public class MailFetcherService {
	private final Logger log = LoggerFactory
			.getLogger(MailFetcherService.class);
	@Inject
			...
			for (ReceivedEmail calEmail : emails) {
				...
				if (calEmail.getAttachments() != null) {
					...
					for (EmailAttachment att : calEmail.getAttachments()) {
						...
						log.debug("Attachment No " + attc++ + " Name : "
								+ att.getName() + " MIME: " + mimeType);
						if (mimeType.equalsIgnoreCase("text/calendar")
							...
							try {
								calendar = new CalendarBuilder()
										.build(new InputStreamReader(att
												.getDataSource()
												.getInputStream()));
								Property method = calendar
										.getProperty("METHOD");

								log.info(method.toString().trim());
								Optional<Meeting> m = meetingService
										.processRequest(
												(MailAddress[]) ArrayUtils.addAll(
														calEmail.getCc(),
														calEmail.getTo()),
												calEmail.getFrom(), calendar,
												calEmail.getSubject(),
												encodingOfEmail);
								if (!m.isPresent()) {
									break;
								...
								log.error("  from " + calEmail.getFrom()
										+ " sent " + calEmail.getSentDate());
							}
